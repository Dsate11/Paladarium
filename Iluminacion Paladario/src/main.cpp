...
const int canalRojo    = 3;
const int canalBlanco2 = 4;
...
static int lastValues[5] = {-1, -1, -1, -1, -1}; // cache de últimos valores por canal
...
void setup() {
  ...
  ledcSetup(canalRojo, frecuencia, resolucion);
  ledcAttachPin(pinRojo, canalRojo);
...
void modoNoche() {
  safeLedcWrite(canalBlanco1, 0);  // blanco tenue
  safeLedcWrite(canalNaranja, 0);  // naranja muy tenue
  safeLedcWrite(canalAzul, 5);     // azul suave
  safeLedcWrite(canalRojo, 0);      // rojo mínimo
  safeLedcWrite(canalBlanco2, 0);  // blanco tenue
}
...
void saveMQTTConfig() {
  ...
  prefs.putString("muser", String(mqttUser));
  prefs.putString("mpass", String(mqttPassw));
  prefs.end();
}
...
void loadMQTTConfig() {
  ...
  String mu = prefs.getString("muser", "");
  String mpw = prefs.getString("mpass", "");
  prefs.end();
  mh.toCharArray(mqttHost, sizeof(mqttHost));
  mqttPort = mp;
  mt.toCharArray(mqttTopic, sizeof(mqttTopic));
  mu.toCharArray(mqttUser, sizeof(mqttUser));
  mpw.toCharArray(mqttPassw, sizeof(mqttPassw));
}
...
bool connectMQTT() {
  ...
  bool ok;
  if (strlen(mqttUser) > 0) ok = mqttClient.connect(clientId.c_str(), mqttUser, mqttPassw);
...
void handleConfig() {
  ...
  html += "<label>Usuario (opcional):</label><input type='text' name='mqtt_user' value='" + String(mqttUser) + "'>";
  html += "<label>Contraseña (opcional):</label><input type='password' name='mqtt_pass' value='" + String(mqttPassw) + "'>";
...
void handleConfigSave() {
  ...
  if (webServer.hasArg("mqtt_pass")) {
    ...
    pass.toCharArray(mqttPassw, sizeof(mqttPassw));
  }
